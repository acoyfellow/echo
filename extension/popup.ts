...
toggle.addEventListener("click", async () => {
  ...
  if (s.open) {
    ...
    const r = (await chrome.runtime.sendMessage({ type: "open" })) as { ok: boolean; error?: string };
...
function friendly(err: string | undefined): string {
  ...
  if (err.startsWith("forbidden_protocol:")) return `cannot run on ${err.split(":")[1]}: pages`;
  if (err.startsWith("unsupported_protocol:")) return "only http/https tabs are supported";
  if (err === "no_active_tab") return "no active tab";
  return err;
}
